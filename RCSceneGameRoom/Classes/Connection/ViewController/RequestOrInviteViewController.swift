...
class RequestOrInviteViewController: UIViewController {
    ...
    private lazy var controllers: [UIViewController] = {
        ...
        let onSeatUserlist = delegate!.onSeatUserlist()
        let onGamePlayerList = delegate!.onGamePlayerList()
        let gameCaptainId = delegate!.gameCaptainPlayer()
        let loginUserRole = delegate!.loginUserRole()
        
        let inviteVC = InviteSeatViewController(roomId: roomId,
                                                loginUserRole: loginUserRole,
                                                onSeatUserList: onSeatUserlist,
                                                onGamePlayerlist: onGamePlayerList,
                                                gameCaptainId: gameCaptainId)
        { [weak self] userId in
            self?.delegate?.inviteVoiceForUser(userId: userId)
        } inviteGameCallback: { [weak self] userId in
            self?.delegate?.inviteGameForUser(userId: userId)
        }
        
       return [inviteVC, requestVC]
    }()
    
        
    init(roomId: String, delegate: HandleRequestSeatProtocol, showPage: Int) {
